...
# ─── استخر اتصال به دیتابیس (Connection Pool) ──────────────────────────────────
# ✅ قبلاً کل برنامه (همه‌ی سلف‌ها + پنل Flask + تایمرهای پس‌زمینه) از یک
# کانکشنِ مشترکِ سراسری با یک Lock استفاده می‌کردن؛ یعنی هر کوئری از هر
# کاربر باید صف می‌کشید پشت همون یک کانکشن — با زیاد شدنِ تعداد کاربرها
# سرعت قطعاً افت می‌کرد (bottleneck سراسری).
# حالا به‌جاش یک ThreadedConnectionPool واقعی داریم: هر Thread/درخواست یک
# کانکشنِ آزاد از استخر می‌گیره، کوئری‌ش رو موازی با بقیه اجرا می‌کنه و
# کانکشن رو برمی‌گردونه. با DB_POOL_MIN / DB_POOL_MAX می‌شه اندازه‌ی استخر
# رو متناسب با بار سرور تنظیم کرد.
import threading
import time

_DB_POOL_MIN = int(os.environ.get("DB_POOL_MIN", "2"))
_DB_POOL_MAX = int(os.environ.get("DB_POOL_MAX", "20"))

# ✅ استخرِ Thread پس‌زمینه برای نوشتن‌های «غیر بحرانی» (مثل تنظیمات) که
# نباید صدازننده رو معطلِ رفت‌وبرگشتِ شبکه به Postgres نگه دارن. اندازه‌ش
# با DB_BG_WRITE_WORKERS قابل تنظیمه.
import concurrent.futures
_bg_executor = concurrent.futures.ThreadPoolExecutor(
    max_workers=int(os.environ.get("DB_BG_WRITE_WORKERS", "8")),
    thread_name_prefix="db-bg-write",
)
        ...
        with _pool_lock:
            if _pool is None:
                _pool = psycopg2.pool.ThreadedConnectionPool(
                    ...
                    # (خیلی از فراهم‌کننده‌های مدیریت‌شده مثلِ Supabase از این
                    # پولرها استفاده می‌کنن) می‌تونه کانکشنِ بی‌کار رو بدونِ
                    # اطلاع‌دادن به کلاینت قطع کنه؛ کیپ‌الایو باعث می‌شه یا
                    # کانکشن زنده بمونه، یا مرگش زودتر (با خطای واضح) کشف بشه.
                    keepalives=1,
                    keepalives_idle=30,
...
def execute_query(query: str, params: tuple = None, fetch_one: bool = False, fetch_all: bool = False, _retry: bool = True):
    """
    اجرای کوئری با گرفتنِ یک کانکشنِ آزاد از استخر (pool) به‌جای اشتراکِ یک
    کانکشنِ سراسری. این کار کوئری‌های همزمانِ کاربرهای مختلف رو از هم مستقل
    می‌کنه و باعث می‌شه سرعت با افزایشِ تعدادِ کاربرها افت نکنه.

    ✅ ری‌ترای خودکار: اگه کانکشنِ گرفته‌شده از استخر خراب/قطع بود (مثلاً
    Supabase idle timeout زده)، همون کانکشن دور ریخته می‌شه و یک‌بار با
    کانکشنِ تازه از استخر دوباره امتحان می‌شه.
    """
    pool = _get_pool()
    conn = pool.getconn()
    conn_broken = False
    try:
        ...
        if conn_broken:
            # کانکشنِ خراب رو کاملاً از استخر حذف کن (به‌جاش استخر خودکار
            # یک کانکشنِ تازه در تلاشِ بعدی می‌سازه)
            try:
...
SETTING_DEFAULTS = {
    ...
    "group_welcome_active": "0",
    "group_welcome_message": "👋 {name} خوش اومدی!",
...
def set_setting(owner_id: int, key: str, value):
    # ✅ الگوی «جواب سریع، نوشتن بعدی»: کش RAM/Redis همین الان (سینک و سریع)
    # آپدیت می‌شه تا هر خواندنِ بعدی (حتی از یک هندلر دیگه، حتی قبل از تمومِ
    # نوشتنِ واقعی) مقدار جدید رو ببینه. نوشتنِ واقعی روی Postgres — که
    # کندترین بخش و یک رفت‌وبرگشتِ شبکه‌ست — روی یک Thread پس‌زمینه انجام
    # می‌شه. یعنی صدازننده (و در نتیجه پاسخِ به کاربر توی تلگرام، چه از
    # دستورِ متنی، چه از دکمه‌ی ربات/هلپر) دیگه منتظرِ دیتابیس نمی‌مونه.
    str_val = str(value)
    _settings_cache[f"{owner_id}:{key}"] = str_val
    try:
        rc.rset(rc.k_setting(owner_id, key), str_val, rc.TTL_SETTING)
    except Exception as e:
        print(f"⚠️ set_setting (کش Redis) error: {e}")
    _bg_executor.submit(_bg_write_setting, owner_id, key, str_val)


def _bg_write_setting(owner_id: int, key: str, str_val: str):
    """نوشتنِ واقعیِ تنظیمات روی Postgres — همیشه در پس‌زمینه اجرا می‌شه."""
    try:
        query = """
        ...
        """
        execute_query(query, (owner_id, key, str_val))
    except Exception as e:
        print(f"❌ set_setting (پس‌زمینه) error: {e}")
