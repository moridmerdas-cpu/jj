...
# ─── لیستِ ایموجی‌های «ساعت پرمیوم» (۱۴۴۰ تا، یکی برای هر دقیقه‌ی شبانه‌روز) ──
try:
...
# ─── شمارشِ پیام‌هایِ روزانه‌یِ هر کاربر (برایِ دستورِ «ایدی») ──────────────────
# فقط در حافظه نگه داشته می‌شه (نه دیتابیس)، چون صرفاً برایِ نمایشِ آماریِ
# لحظه‌ای کافیه و نیازی به ماندگاریِ بینِ ری‌استارت‌ها نداره.
# {(owner_id, sender_id): (تاریخِ امروز به شکلِ رشته, تعداد)}
_daily_msg_counts = {}


def _bump_daily_msg_count(owner_id: int, sender_id: int) -> None:
    today_str = datetime.date.today().isoformat()
    key = (owner_id, sender_id)
    prev = _daily_msg_counts.get(key)
    if prev and prev[0] == today_str:
        _daily_msg_counts[key] = (today_str, prev[1] + 1)
    else:
        _daily_msg_counts[key] = (today_str, 1)


def _get_daily_msg_count(owner_id: int, sender_id: int) -> int:
    today_str = datetime.date.today().isoformat()
    prev = _daily_msg_counts.get((owner_id, sender_id))
    if prev and prev[0] == today_str:
        return prev[1]
    return 0
...
SECRETARY_FAQ_MAX = 6  # حداکثر تعداد سوال آماده‌ی منشی (برای این‌که دکمه‌ها زیاد نشن)


def _get_secretary_faq(owner_id):
    """لیستِ سوال‌های آماده‌ی منشی رو برمی‌گردونه.

    هر آیتم یک دیکشنری {"q": سوال, "a": پاسخِ دستیِ خودِ کاربر} هست.
    برای سازگاری با داده‌ی قدیمی (که فقط لیستِ رشته بود و پاسخش با
    هوش‌مصنوعی ساخته می‌شد)، اگه آیتمِ ذخیره‌شده رشته بود، به شکلِ
    {"q": همون رشته, "a": ""} تبدیل می‌شه (پاسخِ خالی یعنی هنوز کسی
    براش پاسخ دستی تنظیم نکرده).
    """
    import json
    try:
        raw = db.get_setting(owner_id, "secretary_faq", "") or ""
        ...
        for item in data:
            if isinstance(item, dict):
                normalized.append({"q": str(item.get("q", "")), "a": str(item.get("a", ""))})
            else:
                normalized.append({"q": str(item), "a": ""})
        return normalized
    except Exception:
        return []


def _set_secretary_faq(owner_id, items):
    import json
    db.set_setting(owner_id, "secretary_faq", json.dumps(items, ensure_ascii=False))
...
# فونتِ مونواسپیسِ بولد-ایتالیک (همون کدِ FONT["3"]) با فاصله بینِ هر حرف،
# دقیقاً مثلِ چیزی که برایِ «نام کاربر» در کارتِ «ایدی» لازمه: 𝙼 𝙴 𝚁 𝙳 𝙰 𝚂
_MONO_BOLD_ITALIC = "𝙰𝙱𝙲𝙳𝙴𝙵𝙶𝙷𝙸𝙹𝙺𝙻𝙼𝙽𝙾𝙿𝚀𝚁𝚂𝚃𝚄𝚅𝚆𝚇𝚈𝚉𝚊𝚋𝚌𝚍𝚎𝚏𝚐𝚑𝚒𝚓𝚔𝚕𝚖𝚗𝚘𝚙𝚚𝚛𝚜𝚝𝚞𝚟𝚠𝚡𝚢𝚣"
# فونتِ بولدِ سریف (همون FONT["5"])، برایِ امضایِ پایینِ کارتِ «ایدی»
_SERIF_BOLD = "𝐀𝐁𝐂𝐃𝐄𝐅𝐆𝐇𝐈𝐉𝐊𝐋𝐌𝐍𝐎𝐏𝐐𝐑𝐒𝐓𝐔𝐕𝐖𝐗𝐘𝐙𝐚𝐛𝐜𝐝𝐞𝐟𝐠𝐡𝐢𝐣𝐤𝐥𝐦𝐧𝐨𝐩𝐪𝐫𝐬𝐭𝐮𝐯𝐰𝐱𝐲𝐳"


_FULLWIDTH_DIGITS = "０１２３４５６７８９"


def _fullwidth_num(n) -> str:
    """عددهایِ داخلِ کارتِ «ایدی» (تعداد تصاویر پروفایل، پیام‌هایِ امروز) رو
    به رقم‌هایِ Fullwidth تبدیل می‌کنه: 24 → ２４."""
    return "".join(_FULLWIDTH_DIGITS[int(c)] if c.isdigit() else c for c in str(n))


def _spaced_mono_bold(text: str) -> str:
    """نام کاربر رو با فاصله بینِ هر حرف، با فونتِ مونواسپیسِ بولد-ایتالیک
    برمی‌گردونه: مثال «Merdas» → «𝙼 𝙴 𝚁 𝙳 𝙰 𝚂»."""
    converted = _convert_font(text, _MONO_BOLD_ITALIC)
...
def _get_premium_clock_emoji_id(now):
    """document_id ایموجیِ پرمیومِ مخصوصِ همین دقیقه رو برمی‌گردونه (یا None
    اگه لیست هنوز کامل نشده باشه)."""
    idx = now.hour * 60 + now.minute
    if 0 <= idx < len(PREMIUM_CLOCK_EMOJIS):
        return PREMIUM_CLOCK_EMOJIS[idx]
    return None
...
def _register_handlers(cl: TelegramClient, owner_id: int, entry: dict):
    ...
    async def on_incoming(event):
        ...
        if sender_id:
            _bump_daily_msg_count(owner_id, sender_id)
        ...
        if sender_id != owner_id:
            ...
            # ─── اکشنِ خودکار (تایپ/آپلود عکس/ضبط ویس و...) - پورت‌شده از
            # selfsazV5 - وقتی کسی (نه خودِ مالک، نه یک بات) پیام می‌ده، اگه
            # اکشنی فعال باشه، همون‌جا برایِ ۲ ثانیه نشون داده می‌شه ────────
            if not is_bot_sender:
                try:
                    from chat_actions import apply_chat_actions
                    asyncio.ensure_future(apply_chat_actions(cl, owner_id, event.chat_id))
                except Exception:
                    pass
        ...
        if db.get_setting(owner_id, "secretary_active") == "1" and event.is_private and not is_bot_sender:
            ...
            if now - last_reply >= SECRETARY_COOLDOWN:
                ...
                # ─── دکمه‌های سوالات آماده (اگر تنظیم شده باشند) ──────────────
                # چون سلف نمی‌تونه مستقیم پیام با دکمه‌ی شیشه‌ای واقعی بفرسته،
                # از همون ترفندِ inline query به ربات کمکی استفاده می‌کنیم که
                # برای پنل و پیامِ جوینِ اجباری هم استفاده شده: سلف یک inline
                # query به بات کمکی می‌زنه و خودش نتیجه (پیامی با دکمه‌های
                # واقعاً کاربردی) رو کلیک می‌کنه تا توی همین چت با غریبه
                # فرستاده بشه — بدون این‌که غریبه لازم باشه قبلش با بات کمکی
                # استارت زده باشه.
                try:
                    faq_items = _get_secretary_faq(owner_id)
                    if faq_items and config.HELPER_BOT_TOKEN:
                        from helper_bot import get_helper_client
                        helper = get_helper_client()
                        uname = None
                        if helper:
                            try:
                                ...
                                uname = None
                        if uname:
                            results = await cl.inline_query(uname, "منشی")
                            if results:
    ...
    async def on_outgoing(event):
        ...
        # دستورات همیشه فعال
        # ✅ db.set_setting دیگه خودش fire-and-forget است (کش فوری آپدیت
        # می‌شه، نوشتنِ Postgres در پس‌زمینه انجام می‌شه) — پس صدا زدنش
        # صدازننده رو معطل نمی‌کنه و نیازی به کاری اضافه اینجا نیست.
        if text == "سلف روشن":
        ...
        config_commands = [
            ...
            "سوال منشی ", "حذف سوال منشی ", "لیست سوالات منشی",
            "خوش آمد گروه روشن", "خوش آمد گروه خاموش", "پیام خوش آمد گروه ",
            ...
            "ساعت پرمیوم روشن", "ساعت پرمیوم خاموش",
    ...
    # ─── خوش‌آمد خودکار برای اعضای جدید گروه ─────────────────────────────────
    # وقتی «خوش آمد گروه» روشن باشه، به‌محضِ اضافه‌شدنِ یک عضوِ تازه به یک
    # گروه/سوپرگروه که سلف توشه، یک پیامِ خوش‌آمدگویی می‌فرسته. عمداً فقط در
    # گروه‌هایی فعال می‌شه که خودِ سلف توشون ادمینه، نه فقط عضوِ ساده:
    #   ۱) توی گروه‌هایی که سلف صرفاً یک عضوِ عادیه، ارسالِ پیامِ خودکار به
    #      نظرِ ادمین‌های واقعیِ گروه اسپم/مزاحمت می‌رسه.
    #   ۲) این‌جوری کاربر با روشن‌کردنِ این قابلیت توی هر گروهی که هست، خودش
    #      رو مجبورِ اسپم نمی‌کنه — فقط گروه‌هایی که واقعاً ادمینشه فعال می‌شه.
    @cl.on(events.ChatAction())
    async def group_welcome(event):
        if entry.get("paused"):
            return
        if db.get_setting(owner_id, "group_welcome_active", "0") != "1":
            return
        ...
        # 🔒 فقط اگه خودِ سلف توی همین گروه ادمین (یا سازنده) باشه
        try:
            perms = await cl.get_permissions(event.chat_id, "me")
            if not (perms and (perms.is_admin or perms.is_creator)):
                return
        except Exception:
            return
        ...
        welcome_msg = db.get_setting(
            owner_id, "group_welcome_message", "👋 {name} خوش اومدی!"
        )

        try:
            me_id = (await cl.get_me()).id
        except Exception:
            me_id = None
        ...
        for u in users:
            if getattr(u, "bot", False):
                continue
            if me_id is not None and getattr(u, "id", None) == me_id:
                continue  # اضافه‌شدنِ خودِ سلف به گروه رو خوش‌آمد نگو
            name = u.first_name or u.username or "دوست عزیز"
...
async def _handle_command(cl, event, text, owner_id, entry, had_dot=True):
    ...
    if text in ("پنل", "panel"):
        ...
        # ─── ساختِ بنرِ پنل (عکسِ پروفایل + آیدیِ کاربر) ────────────────────────
        # قبلاً اینجا هر بار که کاربر «پنل» می‌نوشت، عکسِ پروفایل از تلگرام
        # دانلود و بنر از نو با PIL ساخته می‌شد - همینِ دانلود+پردازشِ
        # سینکرون (قبل از حتی زدنِ inline query) بود که باعثِ کندیِ باز شدنِ
        # پنل می‌شد. حالا:
        #   ۱) اگه بنرِ همین عکسِ پروفایل از قبل ساخته شده، دوباره ساخته
        #      نمی‌شه - همون مسیرِ کش‌شده مستقیماً استفاده می‌شه.
        #   ۲) ساختِ بنر (وقتی لازمه) در پس‌زمینه انجام می‌شه و جلوی باز شدنِ
        #      پنل رو نمی‌گیره - پنل دقیقاً مثلِ selfsazV5 بلافاصله باز می‌شه؛
        #      دفعه‌ی بعدی که «پنل» نوشته بشه، بنرِ تازه آماده‌ست.
        from panel_banner_cache import get_banner_photo_id

        async def _refresh_banner_in_background(owner_tg_id: int):
            try:
                import tempfile as _tempfile
                _me = await cl.get_me()
                photos = await cl.get_profile_photos("me", limit=1)
                current_photo_id = photos[0].id if photos else None

                if current_photo_id is not None and current_photo_id == get_banner_photo_id(owner_tg_id):
                    return  # عکسِ پروفایل عوض نشده - بنرِ قبلی هنوز معتبره
                ...
                set_banner_path(owner_tg_id, banner_path, photo_id=current_photo_id)
            except Exception as _e:
                print(f"⚠️ خطا در ساختِ بنرِ پنل (پس‌زمینه): {_e}")

        _me_now = await cl.get_me()
        asyncio.ensure_future(_refresh_banner_in_background(_me_now.id))

        from helper_bot import get_helper_client, get_helper_username, start_helper_bot, schedule_panel_timeout
        ...
        while True:
            ...
            # یوزرنیمِ کش‌شده استفاده می‌شه تا مثلِ selfsazV5 هر بار یک
            # get_me جداگانه به تلگرام زده نشه (منبعِ دیگه‌ای از تاخیر).
            helper = get_helper_client()
            uname = get_helper_username() if helper and helper.is_connected() else None

            if not uname:
                try:
                    helper = await start_helper_bot()
                    uname = get_helper_username() if helper else None
                except Exception as e:
                    last_err = str(e)
                    uname = None
    ...
    # (Telethon) انجام می‌شه به‌جای send_chat_action (Pyrogram).
    elif text == "اکشن" or text.startswith("اکشن "):
        from chat_actions import (
            get_persian_action_name, get_english_action_name,
            get_settings, set_action, reset_actions,
        )

        parts = text.split()
        if len(parts) == 1:
            settings = get_settings(owner_id)
            active_names = [get_persian_action_name(k) for k, v in settings.items() if v]
            body = "🎭 سیستم اکشن خودکار\nوضعیت فعلی:\n"
            body += ("فعال: " + "، ".join(active_names)) if active_names else "هیچ اکشنی فعال نیست"
            body += (
                "\n\nدستورات:\n"
                "اکشن لیست - نمایش لیست کامل اکشن‌ها\n"
                "اکشن [نام] روشن - فعال کردن اکشن\n"
                "اکشن [نام] خاموش - غیرفعال کردن اکشن\n"
                "اکشن وضعیت - نمایش وضعیت دقیق\n"
                "اکشن ریست - خاموش کردن همه اکشن‌ها\n\n"
                "مثال:\n"
                "اکشن تایپ روشن\n"
                "اکشن اپلود فایل خاموش"
            )
            await edit(body)
            ...
            if sub_command == "لیست":
                await edit(
                    "🎭 لیست کامل اکشن‌های تلگرام\n\n"
                    "تایپ - در حال تایپ...\n"
                    "اپلود عکس - در حال آپلود عکس...\n"
                    "ضبط ویس - در حال ضبط ویس...\n"
                    "اپلود ویدیو - در حال آپلود ویدیو...\n"
                    "اپلود فایل - در حال آپلود فایل...\n"
                    "ضبط ویدیو - در حال ضبط ویدیو...\n"
                    "اپلود ویس - در حال آپلود ویس...\n"
                    "اپلود ویدیو نوت - در حال آپلود ویدیو نوت...\n"
                    "ضبط ویدیو نوت - در حال ضبط ویدیو نوت...\n"
                    "بازی - در حال بازی...\n"
                    "انتخاب مخاطب - در حال انتخاب مخاطب...\n"
                    "پیدا کردن موقعیت - در حال پیدا کردن موقعیت...\n"
                    "انتخاب استیکر - در حال انتخاب استیکر...\n\n"
                    "نکته: وقتی کسی برات پیام بفرسته، اکشن فعال نشونش داده می‌شه.\n"
                    "اکشن‌ها در پیوی و گروه کار می‌کنند."
                )

            elif sub_command == "وضعیت":
                settings = get_settings(owner_id)
                lines = ["📊 وضعیت دقیق اکشن‌ها\n"]
                for action_name, is_active in settings.items():
                    emoji = "🟢" if is_active else "🔴"
                    persian_name = get_persian_action_name(action_name)
                    lines.append(f"{emoji} {persian_name}: {'فعال' if is_active else 'غیرفعال'}")
                active_count = sum(settings.values())
                lines.append(f"\nآمار: {active_count} از {len(settings)} اکشن فعال")
                await edit("\n".join(lines))

            elif sub_command == "ریست":
                reset_actions(owner_id)
                await edit("✅ همه اکشن‌ها خاموش شدند.")

            else:
                full_text = " ".join(parts[1:])
                if full_text.endswith(" روشن"):
                    action_name_persian = full_text[: -len(" روشن")].strip()
                    action_state = "روشن"
                elif full_text.endswith(" خاموش"):
                    action_name_persian = full_text[: -len(" خاموش")].strip()
                    action_state = "خاموش"
                else:
                    await edit("❌ فرمت دستور نادرست است.\nمثال: اکشن اپلود عکس روشن")
                    return

                action_name = get_english_action_name(action_name_persian)
                persian_name = get_persian_action_name(action_name)
                ok = set_action(owner_id, action_name, action_state == "روشن")
                if not ok:
                    await edit(f"❌ اکشن '{action_name_persian}' یافت نشد.\nاز دستور «اکشن لیست» استفاده کنید.")
                elif action_state == "روشن":
                    await edit(f"✅ اکشن '{persian_name}' فعال شد.\nاز این به بعد وقتی کسی برات پیام بفرسته، این اکشن نشون داده می‌شه.")
                else:
                    await edit(f"✅ اکشن '{persian_name}' غیرفعال شد.")
    ...
    # ─── منشی ────────────────────────────────────────────────────────────────
    ...
    elif text.startswith("سوال منشی "):
        rest = text[len("سوال منشی "):].strip()
        # فرمت: سوال منشی [متنِ روی دکمه] = [پاسخی که بعدِ زدنِ دکمه فرستاده می‌شه]
        if "=" not in rest:
            await edit(
                "⚠️ فرمت درست نیست.\n"
                "از این شکل استفاده کن:\n"
                "`سوال منشی [متن روی دکمه] = [پاسخ بعد از زدن دکمه]`\n\n"
                "مثال:\n"
                "`سوال منشی قیمت چنده؟ = فعلاً ۵۰ هزار تومنه`"
            )
        else:
            q_part, _, a_part = rest.partition("=")
            q = q_part.strip()
            a = a_part.strip()
            if not q:
                await edit("⚠️ متن سوال (قبل از =) خالی است.")
            elif not a:
                await edit("⚠️ متن پاسخ (بعد از =) خالی است.")
            else:
                items = _get_secretary_faq(owner_id)
                if len(items) >= SECRETARY_FAQ_MAX:
                    await edit(f"⚠️ حداکثر {SECRETARY_FAQ_MAX} سوال آماده مجاز است. اول یکی رو حذف کن.")
                else:
                    items.append({"q": q[:60], "a": a[:1000]})
                    _set_secretary_faq(owner_id, items)
                    await edit(f"✅ سوال و پاسخ اضافه شد ({len(items)}/{SECRETARY_FAQ_MAX}).")
    elif text.startswith("حذف سوال منشی "):
        num_str = text[len("حذف سوال منشی "):].strip()
        items = _get_secretary_faq(owner_id)
        try:
            idx = int(num_str) - 1
        except ValueError:
        ...
        if 0 <= idx < len(items):
            ...
            _set_secretary_faq(owner_id, items)
            await edit(f"🗑️ سوال «{removed['q']}» حذف شد.")
        else:
            await edit("⚠️ شماره نامعتبر است.")
    elif text == "لیست سوالات منشی":
        items = _get_secretary_faq(owner_id)
        if not items:
            await edit("📋 هیچ سوال آماده‌ای برای منشی ثبت نشده.")
        else:
            lines = ["📋 سوالات آماده‌ی منشی:\n"]
            for i, item in enumerate(items, 1):
                q, a = item["q"], item["a"]
                a_preview = a if len(a) <= 80 else a[:79] + "…"
                lines.append(f"{i}. ❓ {q}\n   💬 {a_preview}")
            await edit("\n\n".join(lines))

    # ─── خوش‌آمد خودکار گروه ─────────────────────────────────────────────────
    elif text == "خوش آمد گروه روشن":
        ss("group_welcome_active", "1")
        await edit("👋 خوش‌آمد خودکارِ گروه روشن شد.\n💡 فقط توی گروه‌هایی که خودت ادمینشی فعال می‌شه.")
    elif text == "خوش آمد گروه خاموش":
        ss("group_welcome_active", "0"); await edit("👋 خوش‌آمد خودکارِ گروه خاموش شد.")
    elif text.startswith("پیام خوش آمد گروه "):
        ss("group_welcome_message", text[len("پیام خوش آمد گروه "):].strip())
        await edit("✅ پیام خوش‌آمد گروه تنظیم شد.")
    ...
    # ─── ساعت پرمیوم (ایموجی‌استاتوسِ زنده — فقط مخصوصِ اکانت‌های پرمیوم) ─────
    elif text == "ساعت پرمیوم روشن":
        if not PREMIUM_CLOCK_EMOJIS:
            await edit(
                "⚠️ لیستِ ایموجی‌های ساعت پرمیوم هنوز روی سلف تنظیم نشده.\n"
            ...
            if not is_premium:
                await edit(
                    "⛔ ساعت پرمیوم فقط برای اکانت‌های تلگرام پرمیوم فعال می‌شه.\n"
                    "اکانتِ فعلیِ سلف پرمیوم نیست."
                )
            else:
                ss("clock_premium_active", "1")
                await edit("⏰ ساعت پرمیوم روشن شد — وضعیتِ ایموجی هر دقیقه به‌روز می‌شه.")
    # توجه: «ساعت پرمیوم خاموش» توسطِ _EXTRA_TOGGLE_COMMANDS (بالاترِ همین فایل) هندل می‌شه.
    ...
    # ─── ایدی: کارتِ اطلاعاتِ کاربر (عکسِ پروفایل + متنِ فرمت‌شده) ──────────────
    ...
    # بدونِ ریپلای و بدونِ آرگومان = خودِ owner
        ...
        try:
            ...
            display_name = getattr(target, "first_name", None) or getattr(target, "username", None) or str(target.id)
            premium = "فعال" if getattr(target, "premium", False) else "غیرفعال"
            # «اصل سراسری»: نشانِ تاییدِ رسمیِ تلگرام (verified) روی اکانت
            verified = "دارد" if getattr(target, "verified", False) else "ندارد"

            # تعدادِ کلِ عکس‌هایِ پروفایل
            try:
                photos = await cl.get_profile_photos(target, limit=0)
            ...
            today_count = _get_daily_msg_count(owner_id, target.id)

            # ─── ساختِ کارت به سه بخشِ نقل‌قول (blockquote) جدا، دقیقاً مثلِ نمونه ────
            block1 = (
                f"◂ نام کاربر : {_spaced_mono_bold(display_name)}\n"
                f"◂ آیدی عددی : {target.id}\n"
                f"◂ یوزرنیم : {username}\n"
                f"◂ تعداد تصاویر پروفایل : {_fullwidth_num(photo_count)} عدد\n"
                f"◂ اصل سراسری : {verified}\n"
                f"◂پرمیوم: {premium}"
            )
            block2 = (
                "─┅━ پیام های کاربر در روز اخیر ━┅─\n"
                f"◂ پیام های امروز : {_fullwidth_num(today_count)} عدد"
            )
            block3 = _convert_font("NEXO_SELF", _SERIF_BOLD)

            sep = "\n\n"
            caption = block1 + sep + block2 + sep + block3

            from telethon.tl.types import MessageEntityBlockquote
            b1_start = 0
            b2_start = _u16len(block1) + _u16len(sep)
            b3_start = b2_start + _u16len(block2) + _u16len(sep)
            formatting_entities = [
                MessageEntityBlockquote(b1_start, _u16len(block1), collapsed=False),
                MessageEntityBlockquote(b2_start, _u16len(block2), collapsed=False),
                MessageEntityBlockquote(b3_start, _u16len(block3), collapsed=False),
            ]
            ...
            photo_bytes = None
            try:
                photo_bytes = await cl.download_profile_photo(target, file=bytes)
            except Exception:
                photo_bytes = None
            ...
            if photo_bytes:
                ...
                # force_document=False + پسوندِ jpg → همیشه به شکلِ «عکس» ارسال می‌شه، نه فایل
                await cl.send_file(
                    event.chat_id,
                    photo_file,
                    caption=caption,
...
async def _check_force_join_membership(self_client, owner_id: int, user_id: int) -> bool:
    """
    چک می‌کنه که یک کاربرِ خاص (user_id) توی همه‌ی کانال‌های جوین‌اجباریِ
    این owner عضو باشه یا نه — همون منطقِ چکِ عضویتِ داخلِ on_incoming، ولی
    به‌صورتِ یک تابعِ مستقل و قابلِ‌فراخوانی از جای دیگه (مثلاً موقعِ کلیک
    روی دکمه‌های سوالِ آماده‌ی منشی در helper_bot.py) هم استفاده می‌شه.
    اگه جوین اجباری اصلاً کانالی نداشته باشه، True برمی‌گردونه (چیزی برای
    چک کردن نیست).
    """
    fj_channels = _get_force_join_channels(owner_id)
    if not fj_channels:
    ...
    for ch in fj_channels:
        ...
        is_member = False
        try:
            channel_entity = await self_client.get_entity(
                int(ch_id) if str(ch_id).lstrip("-").isdigit() else ch_id
            )
            await self_client(GetParticipantRequest(channel_entity, user_id))
            is_member = True
        except (UserNotParticipantError, KeyError):
            is_member = False
        except ChannelPrivateError:
...
async def _ask_deepseek(knowledge_base: str, question: str) -> str:
    """
    یک سوال از طرف کسی که به سلف پیام داده رو به مدل Groq می‌ده، به‌همراه
    ...
    (اسمِ تابع برای سازگاری با جاهای دیگه‌یِ کد که صداش می‌زنن همون مونده.)
    """
...
def _help_text():
    ...
    sections = [
        ...
        ("🔹 منشی", [
            ...
            "💡 سوالات آماده به‌صورت دکمه زیر پیام منشی نمایش داده می‌شن",
            "💡 پاسخ هر دکمه دقیقاً همونیه که خودت موقعِ ساختنش نوشتی",
        ]),
        ("🔹 خوش‌آمد گروه", [
            "خوش آمد گروه روشن",
            "خوش آمد گروه خاموش",
            "پیام خوش آمد گروه [متن — می‌تونی از {name} برای اسم عضو استفاده کنی]",
            "⚠️ فقط توی گروه‌هایی کار می‌کنه که خودت توشون ادمین باشی",
        ]),
        ("🔹 امنیت", [
            "ضد حذف روشن",
            "ضد حذف خاموش",
        ...
        ("🔹 قفل‌ها", [
            "قفل یوزرنیم روشن / خاموش  ← منشن @username",
            "قفل ریپلای روشن / خاموش",
            "قفل گیف روشن / خاموش",
            "قفل عکس روشن / خاموش",
            "قفل استیکر روشن / خاموش",
            "قفل فوروارد روشن / خاموش",
            "قفل لاگین روشن / خاموش  ← ضد لاگین",
        ]),
        ("🔹 فیلتر کلمات", [
            "فیلتر کلمه [کلمه]",
            "حذف فیلتر کلمه [کلمه]",
        ...
        ("🔹 پاسخ خودکار", [
            ...
            "متن پاسخ خودکار [متن]",
            "پاسخ کلیدی [کلمه] [پاسخ]  ← افزودن یک کلیدواژه",
            "حذف پاسخ کلیدی [کلمه]",
            "لیست پاسخ کلیدی",
            "پاک کردن پاسخ کلیدی",
        ]),
        ...
        ("🔹 اتوماسیون", [
            ...
            "ساعت بیو روشن",
            "ساعت بیو خاموش",
            "ساعت پرمیوم روشن / خاموش  ← ایموجی‌استاتوسِ زنده (فقط اکانت‌های پرمیوم)",
        ]),
        ("🔹 اکشن خودکار", [
            "اکشن  ← نمایش وضعیت و راهنمای اکشن‌ها",
            "تایپینگ روشن / خاموش",
            "گیمینگ روشن / خاموش",
            "ویس روشن / خاموش",
            "ارسال ویدیو روشن / خاموش",
            "💡 وقتی کسی توی چت پیام می‌ده، برای ۲ ثانیه اکشنِ فعال (در حال تایپ/ضبط ویس/...) نشون داده می‌شه",
        ]),
        ("🔹 مدیریت", [
        ...
        ("🔹 ابزار", [
            ...
            "ترجمه [متن]",
            "ترجمه متن  ← ریپلای روی پیام",
            ".هوا [شهر]",
            "ارز  ← دلار، تتر، یورو، پوند",
            "ارز دلار / ارز تتر / ارز یورو / ارز پوند / ارز بیت کوین",
            "لوگو  ← ریپلای روی متن/عکس",
            "پینگ  ← تست سرعت پاسخ",
            ".اسکرین  ← ریپلای، ساخت استیکر از پیام همراه با عکس پروفایل فرستنده",
            ".اسکرین [لینک پست کانال]",
            "تبدیل به گیف  ← ریپلای روی ویدیو",
            "دانلود [لینک پست تلگرام]  ← کپی پست به پیام‌های ذخیره‌شده",
...
PANEL_CATEGORIES = {
    ...
    "clock": {
        ...
        "actions": [
            ("نمایش توضیح ساعت پرمیوم", "INFO::ساعتِ پرمیوم یعنی ایموجی‌استاتوسِ کنارِ اسمت هر دقیقه عوض می‌شه و ساعتِ لحظه‌ای رو نشون می‌ده.\n⚠️ فقط اکانت‌های تلگرام پرمیوم می‌تونن روشنش کنن."),
        ],
    ...
    "actions": {
        ...
        "actions": [],
        "children": [("اکشن به تفکیک گروه", "chat_actions_bygroup")],
    },
    # ─── سطح ۲: لیستِ ۱۳ اکشنِ selfsazV5 - هرکدوم که زده بشه، لیستِ گروه‌ها
    # با دکمه‌ی سبز/قرمز باز می‌شه (special: در helper_bot.py دستی رندر
    # می‌شه چون به دیالوگ‌های زنده‌ی سلف نیاز داره، نه یک لیستِ ثابتِ دستور) ─
    "chat_actions_bygroup": {
        "title": "اکشن به تفکیک گروه",
        "menu_style": "primary",
        "parent": "actions",
        "special": "action_group_root",
        "toggles": [],
        "actions": [],
    },
    "friend_enemy": {
    ...
    "group_welcome": {
        ...
        "menu_style": "success",
        "toggles": [
            ("group_welcome_active", "خوش‌آمد گروه", "خوش آمد گروه روشن", "خوش آمد گروه خاموش"),
        ],
        "actions": [
            ("نمایش متن دستورات خوش‌آمد گروه", "INFO::دستورات خوش‌آمد گروه:\nخوش آمد گروه روشن / خوش آمد گروه خاموش\nپیام خوش آمد گروه [متن دلخواه، از {name} برای اسم عضو استفاده کن]\n⚠️ فقط توی گروه‌هایی کار می‌کنه که خودت توشون ادمین باشی."),
        ],
    },
    "word_filter": {
        "title": "فیلترکلمات",
...
PANEL_CATEGORY_ORDER = [
    ...
    "ping", "logo", "locks",
    "actions", "friend_enemy", "secretary", "group_welcome",
    ...
    while True:
        try:
            ...
            if current_minute != last_minute:
                ...
                # ساعت پرمیوم: وضعیتِ «ایموجی استاتوس» (بج‌ِ کنارِ اسم، مخصوصِ
                # پرمیوم) رو مطابقِ دقیقه‌ی لحظه‌ای عوض می‌کنه — کاملاً مستقل
                # از نام/بیو، چون این یک قابلیتِ جداگانه‌ی تلگرامه (فقط
                # اکانت‌های پرمیوم می‌تونن ایموجی‌استاتوس ست کنن).
                if db.get_setting(owner_id, "clock_premium_active") == "1":
                    emoji_id = _get_premium_clock_emoji_id(now)
                    if emoji_id:
                        try:
                            from telethon.tl.functions.account import UpdateEmojiStatusRequest
                            from telethon.tl.types import EmojiStatus
                            await cl(UpdateEmojiStatusRequest(
                                emoji_status=EmojiStatus(document_id=emoji_id)
                            ))
                            print(f"⏰ [{owner_id}] ایموجی‌استاتوسِ ساعت پرمیوم به‌روز شد: {now.hour:02d}:{now.minute:02d}")
                        except Exception as e:
                            print(f"❌ خطا در به‌روزرسانیِ ساعت پرمیوم: {e}")
                    else:
                        print(f"⚠️ [{owner_id}] لیستِ ایموجی‌های ساعت پرمیوم هنوز کامل نیست ({len(PREMIUM_CLOCK_EMOJIS)}/1440).")
                    ...
                    async for dialog in cl.iter_dialogs(limit=30):
                        ...
                        for key, action in ACTIONS:
                            if db.get_setting(owner_id, key) != "1":
                                continue
                            try:
                                async with cl.action(dialog.entity, action):
                                    await asyncio.sleep(1)
                            except Exception as e:
